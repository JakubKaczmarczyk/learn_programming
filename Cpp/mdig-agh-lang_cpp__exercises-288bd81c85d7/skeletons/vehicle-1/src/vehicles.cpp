...
std::string compute_min_travel_duration_as_string(double distance, const Vehicle& vehicle) {
    std::ostringstream oss;
    oss << std::setprecision(3) << std::fixed << compute_min_travel_duration(distance, vehicle) << " h";
    return oss.str();
}
...
std::vector<Vehicle*> filter_vehicles(
    ...
    std::vector<Vehicle*> result_vec;
    std::copy_if(vehicles_begin,vehicles_end,std::back_inserter(result_vec),
                 [&predicate](const Vehicle* veh_pointer) { return predicate(*veh_pointer);});
    return result_vec;
}
