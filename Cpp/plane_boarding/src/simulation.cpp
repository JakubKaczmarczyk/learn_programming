...
int simulate_board(unsigned int row_nr, unsigned int seats_in_row, QueueAlgorithm algorithm, LuggageTime luggageTime, std::string raport_file_name) {
    Board board(row_nr, seats_in_row);
    board.clear_report(raport_file_name);
    board.create_outer_queue(algorithm, luggageTime);
    int i = 0;
